	...
	if (!isset($_COOKIE["auth"])){
		...
		if($get_user_query->fetch()){
			...
			if(isset($bugid)){
				...
				$close_bug_query = $mysqli->prepare("UPDATE bug set modified = NOW(), status = 2 where id = ?");
				$close_bug_query->bind_param('i',$bugid);
				$close_bug_query->execute();
				$close_bug_query->free_result();
