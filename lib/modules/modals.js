define(['ember','jquery','modules/persona','spinjs','bootstrap'],function(Ember,$,Persona,Spinner){
	...
	/*
	 * Function which will show the admin panel
	 */
	var showAdminModal = function(){
		$('#admin-modal').modal();
	};
	...
	var showNewBugModal = function(model){
	  	...
	  	model.set('newbug.createdby',usr.email.substring(0,usr.email.indexOf('@')));
	  	$('#selectedprojectlabel').text('Select Project');

	  	require('modules/dao').populatePriorities(model);
	  	require('modules/dao').populateLabels(model);
	...
	return {
		...
		showNewBugModal : showNewBugModal,
